# @release-change/logger

## 0.2.0

### Dependencies updates

- @release-change/shared@0.2.0

---

**Full changelog:** [`@release-change/logger@v0.1.3...@release-change/logger@v0.2.0`](https://github.com/release-change/release-change/compare/@release-change/logger@v0.1.3...@release-change/logger@v0.2.0)

## 0.1.3

### Dependencies updates

- @release-change/shared@0.1.3

---

**Full changelog:** [`@release-change/logger@v0.1.2...@release-change/logger@v0.1.3`](https://github.com/release-change/release-change/compare/@release-change/logger@v0.1.2...@release-change/logger@v0.1.3)

## 0.1.2

### Dependencies updates

- @release-change/shared@0.1.2

---

**Full changelog:** [`@release-change/logger@v0.1.1...@release-change/logger@v0.1.2`](https://github.com/release-change/release-change/compare/@release-change/logger@v0.1.1...@release-change/logger@v0.1.2)

## 0.1.1

### Dependencies updates

- @release-change/shared@0.1.1

---

**Full changelog:** [`@release-change/logger@v0.1.0...@release-change/logger@v0.1.1`](https://github.com/release-change/release-change/compare/@release-change/logger@v0.1.0...@release-change/logger@v0.1.1)

## 0.1.0

### Minor changes

- **github:** enhance the way errors are displayed in fail comments ([`8cfd976`](https://github.com/release-change/release-change/commit/8cfd9767a47588c519492132486008ee8b7cdac8))
- **logger:** set scope for logger out of debug mode ([`45760a9`](https://github.com/release-change/release-change/commit/45760a991812e502b55e9c98b3f35ca01b94df30))
- **logger:** enable message logging without any formatting ([`cef13b2`](https://github.com/release-change/release-change/commit/cef13b2d2f6d102252e2ccad4788bb342c21fdc8))

### Patch changes

- **logger:** separate normal scope from debug scope ([`01a6b81`](https://github.com/release-change/release-change/commit/01a6b81ce1b53ba4f054786214802bc8c4037820))

### Dependencies updates

- @release-change/shared@0.1.0
